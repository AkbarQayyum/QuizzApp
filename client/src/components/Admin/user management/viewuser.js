...
function Editdata(e)
{
  let obj={
    ...
    axios.post('/deleteuser',obj).then((res)=>{
        ...
        <TableBody >
          {(rowsPerPage > 0
            ...
            <TableRow key={row._id}>
              ...
              {row.status==="pending"? <TableCell sx={{color:'rgb(18, 95, 91)',fontSize:'100%'}}>
                <button className='button' onClick={(()=>{
                    let obj={id:row._id}
                    axios.post('updateuserstatus',obj).then((res)=>{
                      alert('Status Updated from Pending to Approved');
                    })
                })}>Approve</button>
              </TableCell> :null}
              <TableCell sx={{color:'rgb(18, 95, 91)',fontSize:'100%'}}>
